...
real(rk),intent(in) :: CEU(3),SEU(3),MU(0:180), &
  PHI(0:360),beta
real(rk),intent(in),allocatable :: ACF(:,:)
...
real(rk),intent(in) :: CEU(3),SEU(3),beta
real(rk),intent(in),allocatable :: MU(:),PHI(:),ACF(:,:)
...
real(rk) function RGSAX(ACF,CEU,SEU,mu,phi,beta, &
                                      lmin,lmax)
...
real(rk),intent(in) :: CEU(3),SEU(3),mu,phi,beta
real(rk),intent(in),allocatable :: ACF(:,:)
...
real(rk) function SGSAX(ACF,CEU,SEU,mu,phi,lmin,lmax)
...
real(rk),intent(in),allocatable :: ACF(:,:)
integer :: l
real(rk) :: CEU(3),SEU(3),nu,cphi,sphi,r0,mu0,X(3)
...
real(rk),intent(inout) :: CEU(3),SEU(3)
real(rk),intent(inout),allocatable :: ACF(:,:)
integer :: irnd,l
real(rk) :: alpha,gamma,rn,pi
